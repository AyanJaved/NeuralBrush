...
# Base directory of this script, so paths work no matter where you run it from
...
# Weight paths made relative to this file instead of hardcoded to one machine.
# Adjust the sub-path here if your weights live somewhere else in the repo.
VGG_WEIGHTS_PATH = BASE_DIR /'weights'/ 'vgg_normalised.pth'
DECODER_WEIGHTS_PATH = BASE_DIR / 'weights'/ 'decoder_final.pth'
...
def index():
    ...
    # Note: on GET (first page load), we skip validation/error messages entirely
    # so the form starts clean instead of showing "please upload" errors immediately.
